...
function strToHtml(str) {
  ...
  if (html.substr(0, 2) === '##') {
    ...
    html = `<p class="list"><span class="bullet margin">•</span><span class="margin">${html
      .substr(2)
      .trim()}</span></p>`;
...
<form method="dialog">
    <h1 class="h1">
        ...
        ${
          icon
            ? `<img ${
                usingPluginIcon ? `class="plugin-icon" title="${manifest.name}"` : ''
              } src="${icon}" />`
            : ''
        }
    </h1>
              ...
              (prompt
                ? `<label>${
                    multiline
                      ...
                      : `<input type="text" id="prompt" placeholder="${prompt}" value="${inputVal}" />`
                  }</label>`
                : ''))
        }
    </div>
...
/**
 * Generates a warning message and gives the option to open wakatime
 *
 * @param {string} title
 * @param {string[]} msgs
 * @returns {Promise<{which: number}>} `which` indicates which button was clicked.
 */
async function errorOpenWakatime(title, msg, buttons = ['Open', 'OK']) {
  return createDialog({
    title,
    isError: true,
    msgs: [msg],
 ...
 */
async function prompt(title, msg, prompt, inputVal, buttons = ['Cancel', 'OK'], multiline = false) {
  return createDialog({
    title,
    msgs: [msg],
    prompt,
